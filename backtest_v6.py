...
def test_time_series_momentum(df, lookback=210): # roughly 10 months
    df = df.copy()
    
    buy_signal = df['Close'] > df['Close'].shift(lookback)
...
if __name__ == "__main__":
    ...
    print("Time Series Momentum (10-Month)")
    signals_train = test_time_series_momentum(train_df)
    returns_train = run_backtest(train_df, signals_train)
    print("Train Metrics:", calculate_metrics(returns_train))
    
    signals_test = test_time_series_momentum(test_df)
