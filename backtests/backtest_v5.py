...
def test_price_sma(df, regime=200):
    df = df.copy()
    df['SMA'] = df['Close'].rolling(window=regime).mean()
    
    buy_signal = df['Close'] > df['SMA']
...
if __name__ == "__main__":
    ...
    print("Price > 200 SMA Strategy")
    signals_train = test_price_sma(train_df)
    returns_train = run_backtest(train_df, signals_train)
    print("Train Metrics:", calculate_metrics(returns_train))
    
    signals_test = test_price_sma(test_df)
